...
.vmodel-nav-btn.right{right:4px}
    ...
    cardsContainer.querySelectorAll('.model-card, .model-card-meta, .model-card-wrap').forEach(el => {
      el.addEventListener('click', (ev) => {
        ...
        this.initVideoAspectRatioSlider(); // Инициализируем ползунок соотношений сторон
  ...
  async persistJob(jobId, btn) {
    ...
    try {
      ...
      if (btn) {
        ...
        btn.setAttribute('aria-label', this.isAuthenticated ? 'Сохранено в профиле' : 'Сохранен в галерею');
  ...
  restoreQueue() {
    try {
      ...
      items.forEach(item => {
        ...
        if (this.persistedJobs && this.persistedJobs.has(String(item.job_id))) {
          // Удаляем из очереди, т.к. она уже в профиле
          toRemove.push(item.job_id);
          return;
        }

        // 3. СТРОГАЯ ПРОВЕРКА: если задача создана ДО момента очистки очереди - удаляем
        ...
        // 5. Skip legacy mixed-in IMAGE entries (no video_url but has image_url)
  ...
  /**
   * Инициализация ползунка соотношения сторон для видео
   */
  async initVideoAspectRatioSlider() {
    if (!this.selectedModel) return;
    ...
    try {
      ...
      this.videoAspectRatioSlider = new AspectRatioSlider({
        container: container,
        modelType: 'video',
  ...
  /**
   * Обновление отображения размеров видео
   */
  updateVideoDimensionsDisplay() {
    const dimensionsInfo = document.getElementById('video-dimensions-info');
    const dimensionsDisplay = document.getElementById('video-dimensions-display');
    const widthInput = document.getElementById('video-resolution-width');
    const heightInput = document.getElementById('video-resolution-height');
  ...
  async generateVideoMulti() {
    ...
    const duration = document.getElementById('video-duration')?.value || 5;
    const width = document.getElementById('video-resolution-width')?.value;
    const height = document.getElementById('video-resolution-height')?.value;
